...
<table class="table dtable-striped table-hover no-head-border" border="1" style="border:solid; border-color: black; border-width: thin;">
 ...
 <th style="border:solid; border-width: thin; border-color: #eee; color: white; background-color: #0060a0;">Referrals</th>
...
$sql=$con->query("SELECT * FROM referral GROUP BY referral_email") or die("Error2 : ". mysqli_error($con));
 ...
 $i=1;
  ...
  while ($rows=mysqli_fetch_array($sql))
   {
    $id=$rows['id'];
    $referral_email = $rows['referral_email'];
    ...
    $sqlx =$con->query("SELECT * FROM referral WHERE referral_email = '$referral_email'") or die("Error2 : ". mysqli_error($con));
    $countx = mysqli_num_rows($sqlx);

 $sqlr =$con->query("SELECT * FROM customers WHERE email = '$referral_email'") or die("Error2 : ". mysqli_error($con));
...
?>
<tr><td style="border:solid; border-width: thin; border-color: #eee;"><?php echo $i; ?><td style="border:solid; border-width: thin; border-color: #eee;"><?php echo $fname; ?> <?php echo $lname; ?><td style="border:solid; border-width: thin; border-color: #eee;"><?php echo $countx; ?><td style="border:solid; border-width: thin; border-color: #eee;"><a href="?p=view_ref&e=<?php echo $referral_email; ?>"><button class="btn btn-warning">View</button> </a></td></td></td></tr>
